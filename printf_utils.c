...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/05 16:19:06 by akhachat          #+#    #+#             */
/*   Updated: 2021/04/19 16:58:31 by akhachat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_printf_d(va_list argument, t_flags *flag)
{
	...
	zeros = (flag->dot - ft_int_len(a, 10) < 0 ? 0 :
		flag->dot - ft_int_len(a, 10));
	spaces = ((a == 0 && flag->dot == 0)
	? flag->width - zeros : flag->width - ft_int_len(a, 10) - zeros);
	if (a < 0)
		spaces--;
...
int			ft_printf_u(va_list argument, t_flags *flag)
{
	...
	zeros = (flag->dot - ft_int_len_u(a, 10) < 0 ?
		0 : flag->dot - ft_int_len_u(a, 10));
	spaces = ((a == 0 && flag->dot == 0)
	? flag->width - zeros : flag->width - ft_int_len_u(a, 10) - zeros);
	if (spaces > 0 && flag->minus == 0 && flag->zero == 0)
		while (spaces--)
			out += ft_putchar(' ');
	if (flag->dot >= 0)
